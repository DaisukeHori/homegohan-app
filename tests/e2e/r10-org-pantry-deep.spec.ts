...
test.describe("A. Org — 認可 & ページレンダリング", () => {
  ...
  /**
   * A-6: /api/org/members 未認証 → 401
   */
  test("A-6: /api/org/members 未認証 → 401", async ({ page }) => {
    await page.goto("/login");
    const { status } = await apiGet(page, "/api/org/members");
    expect(status).toBe(401);
  });
...
test.describe("A. Org — メンバー追加 API バリデーション", () => {
  /**
   * A-14: /api/org/members POST 必須フィールド欠如 → 400 or 403
   */
  test("A-14: /api/org/members POST 必須フィールド欠如 → 400/403", async ({
    authedPage,
  }) => {
    await authedPage.goto("/home");
    const { status } = await apiPost(authedPage, "/api/org/members", {
  ...
  /**
   * A-15: /api/org/members POST 不正メールアドレス形式 → 400 or 403
   */
  test("A-15: /api/org/members POST 不正メール → 400/403", async ({ authedPage }) => {
    await authedPage.goto("/home");
    const { status } = await apiPost(authedPage, "/api/org/members", {
      email: "not-an-email",
