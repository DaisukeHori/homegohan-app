...
test.describe("究極モードが Premium プラン準備中としてロックされている", () => {
  test("究極モードトグルが disabled であり有効化できない", async ({
    ...
    const assistantButton = tourPendingUser
    ...
    // モーダルが開くのを待機
    const modal = tourPendingUser.locator('[role="dialog"], [data-testid="v4-generate-modal"]').first();
    const modalOpened = await modal
      .waitFor({ state: "visible", timeout: 10_000 })
      .then(() => true)
      .catch(async () => {
        // role=dialog がない場合はモーダルらしき要素を探す
        const altModal = tourPendingUser.locator('text=究極モード').first();
        return altModal
          .waitFor({ state: "visible", timeout: 10_000 })
          .then(() => true)
          .catch(() => false);
    ...
    // 究極モードトグルを確認
    const ultimateToggle = tourPendingUser.locator('[data-testid="ultimate-mode-toggle"]');
    ...
    if (!toggleVisible) {
      ...
      const ultimateModeSection = tourPendingUser.locator('text=究極モード').locator('..');
      await expect(ultimateModeSection).toBeVisible({ timeout: 5_000 });
      // 準備中ラベルが表示されていることを確認
      await expect(tourPendingUser.locator('text=準備中').first()).toBeVisible();
      return;
    }
  ...
  test("究極モードの「Premium」バッジが引き続き表示される", async ({
    ...
    const premiumBadge = tourPendingUser.locator('text=Premium').first();
