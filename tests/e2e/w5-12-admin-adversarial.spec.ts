...
test("[admin][adversarial] D-17: 監査ログ取得 → 通常 user は 403", async ({
  ...
  await authedPage.goto(`${BASE_URL}/home`);
  const result = await apiFetch(authedPage, "/api/super-admin/audit-logs");
  expect(result.status).toBe(403);
});
...
test("[admin][adversarial] E-20: お問い合わせ一覧 → 通常 user は 403", async ({
  ...
  await authedPage.goto(`${BASE_URL}/home`);
  const result = await apiFetch(authedPage, "/api/admin/support/tickets");
  expect(result.status).toBe(403);
});
...
test("[admin][adversarial] F-23: 財務ダッシュボード → 通常 user は 403", async ({
  ...
  await authedPage.goto(`${BASE_URL}/home`);
  const result = await apiFetch(authedPage, "/api/admin/finance/dashboard");
  expect(result.status).toBe(403);
});
...
test("[admin][adversarial] F-24: 財務エクスポート → 通常 user は 403", async ({
  ...
  await authedPage.goto(`${BASE_URL}/home`);
  const result = await apiFetch(authedPage, "/api/admin/finance/exports");
  expect(result.status).toBe(403);
});
...
test("[super-admin][adversarial] F-25: クーポン一覧 → 通常 user は 403", async ({
  ...
  await authedPage.goto(`${BASE_URL}/home`);
  const result = await apiFetch(authedPage, "/api/super-admin/coupons");
  expect(result.status).toBe(403);
});
...
test("[super-admin][adversarial] F-26: A/B 実験一覧 → 通常 user は 403", async ({
  ...
  await authedPage.goto(`${BASE_URL}/home`);
  const result = await apiFetch(authedPage, "/api/super-admin/experiments");
  expect(result.status).toBe(403);
});
...
test("[super-admin][adversarial] H-34: Feature flags 取得 → super_admin のみ", async ({
  ...
  await authedPage.goto(`${BASE_URL}/home`);
  const result = await apiFetch(authedPage, "/api/super-admin/flags");
  expect(result.status).toBe(403);
});
