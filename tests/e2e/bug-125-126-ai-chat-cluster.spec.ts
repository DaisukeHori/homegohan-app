...
test.describe("Bug-125: フローティングボタン連続クリック二重表示防止", () => {
  ...
  test("チャットを閉じて再度開いても正常に動作する", async ({
    ...
    await authedPage.locator('button[aria-label="AIチャットを閉じる"]').click().catch(async () => {
      // フォールバック: lucide-x クラスを持つ SVG の親ボタン
      await authedPage.locator('button').filter({ has: authedPage.locator('svg.lucide-x') }).last().click().catch(async () => {
