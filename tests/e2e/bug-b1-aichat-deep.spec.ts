...
test.describe("シナリオ 1: 会話継続 10往復", () => {
  test("10往復のメッセージを送受信できる (Bug#94 候補)", async ({
    ...
    await openChatWindow(tourPendingUser);
    ...
    for (let i = 1; i <= TURNS; i++) {
      ...
      const bubbles = await sendAndWaitForResponse(tourPendingUser, msg, 35_000);
      ...
      const sendBtn = tourPendingUser.locator('button').filter({
    ...
    await saveScreenshot(tourPendingUser, evidenceDir, "10turns-final");

    // 最終的に AI バブルが TURNS 回以上存在すること
    const finalBubbles = tourPendingUser
      .locator('[data-testid="ai-message-bubble"]')
      .filter({ hasNotText: "" });
...
test.describe("シナリオ 2: コンテキスト切替", () => {
  test("食事 → 無関係な話題 → 食事の文脈へ戻れる (Bug#95 候補)", async ({
    ...
    await saveScreenshot(tourPendingUser, evidenceDir, "step1-curry-context");
    ...
    await saveScreenshot(tourPendingUser, evidenceDir, "step2-weather-question");
    ...
    await saveScreenshot(tourPendingUser, evidenceDir, "step3-back-to-food");
...
test.describe("シナリオ 3: 絵文字 + 特殊文字", () => {
  test("絵文字・全角半角混在メッセージを送信できる (Bug#96 候補)", async ({
    ...
    await saveScreenshot(tourPendingUser, evidenceDir, "emoji-input");

    await input.press("Enter");

    // ユーザーメッセージが表示される（絵文字を含む）
    await expect(
      tourPendingUser.locator("text=🍎").first()
    ).toBeVisible({ timeout: 10_000 });
    ...
    await saveScreenshot(tourPendingUser, evidenceDir, "emoji-response");

    // コンソールエラーがないことを確認（クリティカルなものだけ）
    const consoleErrors: string[] = [];
    tourPendingUser.on("console", (msg) => {
      if (msg.type() === "error" && !msg.text().includes("favicon")) {
        consoleErrors.push(msg.text());
      }
    });
    // 200ms 待ってエラーがないか確認
    await tourPendingUser.waitForTimeout(200);
    // エラーログを記録（failではなく観察）
...
test.describe("シナリオ 4: 送信ボタン連打", () => {
  test("送信中は2重送信されない — isSendingフラグで防止 (Bug#97 候補)", async ({
    ...
    await saveScreenshot(tourPendingUser, evidenceDir, "rapid-enter-pressed");

    // ユーザーメッセージが表示される
    await expect(
      tourPendingUser.locator("text=連打テストメッセージです").first()
    ).toBeVisible({ timeout: 5_000 });
    ...
    const userMessages = tourPendingUser.locator("text=連打テストメッセージです");
    const userMsgCount = await userMessages.count();
    console.log("[連打テスト] ユーザーメッセージ数:", userMsgCount);

    await saveScreenshot(tourPendingUser, evidenceDir, "after-rapid-send");

    // 重複送信されていないはず（1件のみ）
    if (userMsgCount > 1) {
      // バグとして記録
      await saveScreenshot(tourPendingUser, evidenceDir, "BUG-duplicate-messages");
      console.error(
...
test.describe("シナリオ 5: チャット履歴の永続化", () => {
  test("チャットを閉じて再度開くと履歴が表示される (Bug#98 候補)", async ({
    ...
    // メッセージを送信
    await sendAndWaitForResponse(tourPendingUser, "履歴テスト: 覚えていますか？", 35_000);

    await saveScreenshot(tourPendingUser, evidenceDir, "step1-message-sent");

    // チャットウィンドウを閉じる (X ボタン)
    const chatWindow = tourPendingUser.locator(".fixed.bottom-24.right-4");
    const closeBtn = chatWindow.locator("button.rounded-full:not([disabled])").last();
    await closeBtn.click({ force: true });

    // floating button が戻ること
    const floatingBtn = tourPendingUser.getByTestId("ai-chat-floating-button");
    await expect(floatingBtn).toBeVisible({ timeout: 8_000 });

    await saveScreenshot(tourPendingUser, evidenceDir, "step2-chat-closed");
    ...
    // セッション読み込みを待つ
    await tourPendingUser.waitForTimeout(3_000);

    await saveScreenshot(tourPendingUser, evidenceDir, "step3-chat-reopened");

    // 以前のメッセージが残っているか確認
    const previousMsg = tourPendingUser.locator("text=履歴テスト").first();
    ...
    if (!isVisible) {
      ...
      await saveScreenshot(tourPendingUser, evidenceDir, "BUG-no-history");
    } else {
...
test.describe("シナリオ 6: 複数セッション", () => {
  test("新規セッションを作成して切り替えられる (Bug#99 候補)", async ({
    ...
    // セッション1でメッセージを送信
    await sendAndWaitForResponse(tourPendingUser, "セッション1のメッセージです", 35_000);

    await saveScreenshot(tourPendingUser, evidenceDir, "session1-message");
    ...
    // セッションリストが開く
    await expect(
      tourPendingUser.getByRole("button", { name: /新しい相談を始める/ })
    ).toBeVisible({ timeout: 5_000 });

    await saveScreenshot(tourPendingUser, evidenceDir, "session-list-open");

    // 新しいセッションを開始
    await tourPendingUser.getByRole("button", { name: /新しい相談を始める/ }).click();

    // 新しいセッションの初期メッセージが表示される
    await expect(
      tourPendingUser.locator('[data-testid="ai-message-bubble"]').first()
    ).toBeVisible({ timeout: 10_000 });

    await saveScreenshot(tourPendingUser, evidenceDir, "new-session-created");

    // セッション2でメッセージを送信
    await sendAndWaitForResponse(tourPendingUser, "セッション2のメッセージです", 35_000);

    await saveScreenshot(tourPendingUser, evidenceDir, "session2-message");
    ...
    const sessionButtons = tourPendingUser.locator(
    ...
    await saveScreenshot(tourPendingUser, evidenceDir, "session-list-with-sessions");
...
test.describe("シナリオ 7: 長文送信", () => {
  test("2000文字メッセージでタイムアウト/クラッシュが起きない (Bug#100 候補)", async ({
    ...
    await saveScreenshot(tourPendingUser, evidenceDir, "long-message-input");

    await input.press("Enter");

    // ユーザーメッセージが表示される (先頭20文字で確認)
    await expect(
      tourPendingUser.locator(`text=${longMessage.slice(0, 10)}`).first()
    ).toBeVisible({ timeout: 10_000 });
    ...
    await saveScreenshot(tourPendingUser, evidenceDir, "long-message-response");

    if (!responded) {
      console.error(
        "[BUG#100] 長文メッセージ送信後 35 秒以内に AI 応答がない（タイムアウト）"
      );
      await saveScreenshot(tourPendingUser, evidenceDir, "BUG-long-message-timeout");
    }
...
test.describe("シナリオ 8: セッション終了と要約", () => {
  test("Archive ボタンでセッションを終了すると要約が表示される", async ({
    ...
    const summaryOrWelcome = tourPendingUser
      .locator('[data-testid="ai-message-bubble"]')
      .filter({ hasNotText: "" });
...
test.describe("シナリオ 9: ナビゲート離脱後の再開", () => {
  test("別ページへ移動して戻ってきてもチャットが正常に再開できる", async ({
    ...
    await tourPendingUser.waitForTimeout(2_000);

    // /home へ戻る
    await tourPendingUser.goto("/home");
    await tourPendingUser.waitForTimeout(1_000);

    // フローティングボタンが表示されること
    const btn = tourPendingUser.getByTestId("ai-chat-floating-button");
    await expect(btn).toBeVisible({ timeout: 15_000 });
    ...
    // 新しいメッセージを送信できること
    await sendAndWaitForResponse(tourPendingUser, "再開テストメッセージです", 35_000);
...
test.describe("シナリオ 10: Suggested prompts", () => {
  test("suggested prompts をクリックして AI 応答が来る", async ({
    ...
    for (const prompt of suggestedPrompts) {
      const btn = tourPendingUser.getByRole("button", { name: prompt, exact: true });
      ...
      if (isVisible) {
        ...
        // ユーザーメッセージが表示される
        await expect(
          tourPendingUser.locator(`text=${prompt.slice(0, 10)}`).first()
        ).toBeVisible({ timeout: 5_000 });
...
test.describe("シナリオ 12: openChat 連続呼び出し", () => {
  test("floating button 連続クリックで二重セッションが作成されない", async ({
    ...
    const chatWindows = await tourPendingUser
