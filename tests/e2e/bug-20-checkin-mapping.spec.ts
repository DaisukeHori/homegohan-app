...
test.describe("Bug-20: 30秒チェックイン → health_records マッピング検証", () => {
  test("チェックイン送信後、mood_score は書き換えられない (null または既存値のまま)", async ({
    ...
    const beforeRes = await tourPendingUser.request.get(`/api/health/records/${TODAY}`);
    ...
    if (!alreadyDone) {
      ...
      const submitButton = tourPendingUser.getByRole("button", { name: /チェックイン完了/ });
    ...
    const afterRes = await tourPendingUser.request.get(`/api/health/records/${TODAY}`);
  ...
  test("/api/health/records/quick は mood_score を送信しない限り更新しない", async ({
    ...
    const quickRes = await tourPendingUser.request.post("/api/health/records/quick", {
      data: {
        record_date: TODAY,
        sleep_quality: 3,
    ...
    const finalRes = await tourPendingUser.request.get(`/api/health/records/${TODAY}`);
