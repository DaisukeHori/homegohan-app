...
test("health page today's record cards have visible labels (or empty-state CTA)", async ({ tourPendingUser }) => {
  ...
  const weightLabel = tourPendingUser.getByText("体重", { exact: true });
  const ctaButton = tourPendingUser.getByText("今日の記録をつける", { exact: true });
  ...
  if (hasData) {
    ...
    await expect(tourPendingUser.getByText("気分", { exact: true }).first()).toBeVisible();
    await expect(tourPendingUser.getByText(/睡眠時間|睡眠の質/).first()).toBeVisible();
  } else {
