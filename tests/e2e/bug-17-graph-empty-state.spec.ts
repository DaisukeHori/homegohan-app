...
test.describe("health graphs empty state", () => {
  test("does not show hardcoded '最大 100.0' when no data exists", async ({ tourPendingUser }) => {
    await tourPendingUser.goto("/health/graphs");
    await expect(tourPendingUser.getByRole("heading", { name: "推移グラフ" })).toBeVisible({
      timeout: 15_000,
    });

    for (const tabLabel of METRIC_TABS) {
      const tab = tourPendingUser.getByRole("button", { name: tabLabel }).first();
      ...
      await tourPendingUser.waitForLoadState("networkidle").catch(() => {});

      const emptyMessage = tourPendingUser.getByText("データがありません").first();
