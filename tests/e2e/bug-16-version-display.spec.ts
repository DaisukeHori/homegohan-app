...
test.describe("settings version display (Bug-16)", () => {
  test("footer shows current year and not the stale 2025 copyright", async ({ tourPendingUser }) => {
    await tourPendingUser.goto("/settings");

    // ページが読み込まれるまで待機
    await expect(tourPendingUser.getByRole("heading", { name: /設定/ }).first()).toBeVisible({
      timeout: 15_000,
    });

    // バージョン情報テキストを取得
    const versionText = tourPendingUser.getByText(/ほめゴハン/).last();
    ...
    const pageContent = await tourPendingUser.content();
