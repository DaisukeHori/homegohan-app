...
test.describe("Bug #82: body_stats height/weight min/max バリデーション", () => {
  test("身長に 0 を入力したとき「次へ」が disabled になる", async ({ page }) => {
    ...
    // リセットして onboarding フローに入る
    await page.request.delete(`${BASE_URL}/api/onboarding/status`);
...
test.describe("Bug #79: リセット時に onboarding_completed_at がクリアされる", () => {
  test("DELETE /api/onboarding/status がステータスを not_started に戻す", async ({ page }) => {
    ...
    // リセット API を呼び出す
    const response = await page.request.delete(`${BASE_URL}/api/onboarding/status`);
    expect(response.status()).toBe(200);

    const body = await response.json();
    expect(body.success).toBe(true);

    // リセット後にステータスを取得して not_started になっていることを確認
    const statusResponse = await page.request.get(`${BASE_URL}/api/onboarding/status`);
