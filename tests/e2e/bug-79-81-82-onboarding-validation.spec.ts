...
test.describe("Bug #82: body_stats height/weight min/max バリデーション", () => {
  test("身長に 0 を入力したとき「次へ」が disabled になる", async ({ page }) => {
    ...
    if (!EMAIL || !PASSWORD) {
      // Why: 環境変数 E2E_USER_EMAIL / E2E_USER_PASSWORD が未設定
      // 解除条件: CI/ローカルに E2E_USER_EMAIL・E2E_USER_PASSWORD を設定後
      test.fixme(true, "C: E2E_USER_EMAIL / E2E_USER_PASSWORD が未設定。環境変数を設定後に解除可能。");
      return;
    }
...
test.describe("Bug #79: リセット時に onboarding_completed_at がクリアされる", () => {
  test("DELETE /api/onboarding/status がステータスを not_started に戻す", async ({ page }) => {
    ...
    if (page.url().includes('/login')) {
      // Why: signIn が失敗してログインページのままになっている (認証情報・サーバー状態の問題)
      // 解除条件: E2E_USER_EMAIL / E2E_USER_PASSWORD が正しく、認証サーバーが正常動作している状態
      test.fixme(true, "C: signIn 失敗。認証情報と認証サーバーの状態確認後に解除可能。");
      return;
    }
