...
test.describe("Bug 4 リグレッション: signup → 確認メール URL スキーム検証", () => {
  ...
  // hashed_token を token_hash パラメータとして /auth/callback に渡すことで
  // メール確認フローをシミュレートする
  // ────────────────────────────────────────────────────────
  test("確認 URL 訪問 → /auth/callback → /onboarding または /home への完全フロー", async ({
    page,
  }) => {
    const uniqueEmail = `e2e-signup-callback-${Date.now()}@homegohan.test`;
    const password = "TestE2E2026!secure";
    const adminClient = getAdminClient();
    const baseURL = process.env.PLAYWRIGHT_BASE_URL ?? "http://localhost:3000";
    ...
    try {
      // 1. admin generateLink でユーザー作成 & 確認 URL のトークンを取得
      const { data, error } = await adminClient.auth.admin.generateLink({
        type: "signup",
        email: uniqueEmail,
        password,
      ...
      // hashed_token は /auth/callback?token_hash=...&type=signup で使用できる
      const hashedToken = data?.properties?.hashed_token;
      console.log(`[test3] hashed_token present: ${!!hashedToken}`);
      expect(hashedToken).toBeTruthy();

      // 2. /auth/callback に token_hash と type を付けてアクセス (メール確認フローのシミュレーション)
      const callbackUrl = `${baseURL}/auth/callback?token_hash=${hashedToken}&type=signup`;
      console.log(`[test3] navigating to: ${callbackUrl}`);

      await page.goto(callbackUrl);

      // 3. /auth/callback が認証処理を行い onboarding/home のいずれかへリダイレクトすること
      ...
      console.log(`[test3] final URL: ${finalUrl}`);
      ...
      expect(finalPath).toMatch(/^\/(onboarding|home|admin)/);

      console.log(`[test3] PASS: ${uniqueEmail} → /auth/callback → ${finalPath} の遷移を確認`);
    } finally {
      // 4. cleanup: テストユーザーを削除
      if (createdUserId) {
        const { error: deleteError } = await adminClient.auth.admin.deleteUser(createdUserId);
        if (deleteError) {
          console.warn(`[cleanup] deleteUser エラー: ${deleteError.message}`);
  ...
  test("/signup UI から signup → /auth/verify へ遷移すること", async ({ page }) => {
    ...
    try {
      // global-setup が設定した既認証セッションをクリア
      // (既ログイン状態で /signup にアクセスすると Internal Server Error になる場合がある)
