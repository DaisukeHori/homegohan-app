...
test.describe("Bug #64: 定食写真がオートモードで meal として分類される", () => {
  ...
  test("meal-1.jpg (定食) → result ステップへ遷移し classify-failed にならない", async ({
    ...
    await tourPendingUser.goto("/meals/new");

    // mode-select ステップ確認
    await expect(tourPendingUser.getByText("撮影するものを選んでください")).toBeVisible({
      timeout: 15_000,
    });

    // オートモード (デフォルト) で進む
    await tourPendingUser.getByRole("button", { name: "撮影へ進む" }).click();

    // capture ステップ: file input を待つ
    const fileInput = tourPendingUser.locator('input[type="file"][multiple]');
    ...
    // 解析ボタンをクリック
    const analyzeButton = tourPendingUser.getByRole("button", { name: /AIが判別して解析/ });
    await expect(analyzeButton).toBeVisible({ timeout: 10_000 });
    await analyzeButton.click();

    // 「解析結果」ヘッダが出るまで最大 90 秒待つ
    await expect(tourPendingUser.getByText("解析結果").first()).toBeVisible({
      timeout: 90_000,
    }).catch(async (originalError) => {
      const classifyFailed = await tourPendingUser
...
test.describe("Bug #65: classify-failed 遷移中に analyzing UI が残存しない", () => {
  ...
  test.skip("analyzing → classify-failed 遷移時に analyzing スピナーが残らない", async ({
    ...
    await expect(tourPendingUser.getByText(/AIが写真の種類を確認中/)).toBeVisible({ timeout: 15_000 });

    // 次のステップ (result or classify-failed) に遷移した直後
    // analyzing UI のテキストが消えていることを確認
    await expect(tourPendingUser.getByText(/AIが写真の種類を確認中/)).not.toBeVisible({
      timeout: 90_000,
    });

    // この時点でスピナーが残存していないこと (animate-spin を持つ div が消えている)
    const spinner = tourPendingUser.locator(".animate-spin");
