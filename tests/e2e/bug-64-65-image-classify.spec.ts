...
test.describe("Bug #64: 定食写真がオートモードで meal として分類される", () => {
  // Why: LLM 出力が決定論的でないため CI で flaky。prompt を継続改善中 (#64)。
  // 解除条件: LLM の分類精度が安定して meal-1.jpg が常に result ステップに遷移することを
  //   連続 10 回以上確認できた場合。または API モックで分類結果を固定する仕組みを導入後。
  test.fixme(
    true,
    'C: LLM 出力が非決定論的で CI flaky。分類精度安定または API モック導入後に解除可能。',
  );
...
test.describe("Bug #65: classify-failed 遷移中に analyzing UI が残存しない", () => {
  // Why: bug-64 修正により meal-1.jpg が正常に meal 分類されるようになったため
  //   classify-failed を再現できなくなった。別 fixture が必要。
  // 解除条件: classify-failed を確実に引き起こす専用 fixture 画像を /tmp/classify-test/ に配置後
  test.fixme("analyzing → classify-failed 遷移時に analyzing スピナーが残らない", async ({
    authedPage,
  }) => {
    await authedPage.goto("/meals/new");
    await expect(authedPage.getByText("撮影するものを選んでください")).toBeVisible({
