...
describe('POST /api/super-admin/plans/[id]/price-change (#1041 round-4 C: 旧 Price deactivate 結果の監査ログ記録)', () => {
  function auditLogInsertPayload(fake: ReturnType<typeof createFakeSupabase>): Record<string, unknown> {
    const idx = fake.from.mock.calls.findIndex((c) => c[0] === 'admin_audit_logs');
    const builder = fake.from.mock.results[idx]!.value as { insert: ReturnType<typeof vi.fn> };
  ...
  it('Edge Function が deactivated:true を返した場合、監査ログに old_stripe_price_deactivated:true が記録される', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({ new_stripe_price_id: 'price_new', deactivated: true }),
      }),
    );
  ...
  it('Edge Function が interval 不一致で deactivate をスキップした場合、監査ログに理由が記録される (①→② シナリオ)', async () => {
    process.env.STRIPE_SECRET_KEY = 'sk_test_x';
    fakeSupabase = createFakeSupabase({
      subscription_plans: [{ data: existingPlan(), error: null }],
      plan_price_history: [{ data: null, error: null }],
    ...
    expect(details.old_stripe_price_deactivation_skipped_reason).toBe('interval_mismatch');
  });
});
