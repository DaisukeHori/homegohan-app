...
| 中断で API リトライ攻撃 | meals に大量 INSERT | 中 |
...
```sql
CREATE OR REPLACE FUNCTION check_meals_sandbox_eligibility()
RETURNS trigger AS $$
DECLARE
...
CREATE TRIGGER meals_sandbox_check
  BEFORE INSERT ON meals
  FOR EACH ROW
  EXECUTE FUNCTION check_meals_sandbox_eligibility();
```
...
問題:
- `meals` に sandbox=true で 2 件目 INSERT
...
async function handleSaveSandbox() {
  // 直近 5 分以内に sandbox meal があれば skip
  const recent = await fetch('/api/meals?limit=5&user_only=true&since=5min&is_sandbox=true');
...
```sql
-- meals に partial unique index
CREATE UNIQUE INDEX uniq_user_sandbox_meal
  ON meals (user_id, is_sandbox)
...
ただ unique 制約は `user_daily_meals` にも追加する必要があり、メニュー側はもう少し複雑。

#### v1 推奨方針

- Step 1: meals unique 制約 (`user_id, is_sandbox=true` で 1 行 = `uniq_user_sandbox_meal`)
- Step 2: user_daily_meals unique 制約 (同上 = `uniq_user_sandbox_daily_meal`)
...
```sql
-- 既存 (user_profiles の PK は id、auth.users(id) FK)
...
### 5.2 meals / user_daily_meals

既存 RLS:

```sql
CREATE POLICY meals_owner_rw ON meals
...
describe('sandbox=true 偽装防止', () => {
  ...
  it('既存ユーザー (meals あり) が sandbox=true → 409', async () => {
    await insertMeal(userId, { is_sandbox: false });
    const res = await postMealPhoto(userId, { sandbox: true, ... });
    expect(res.status).toBe(409);
  });
...
#### 検出
- 監視: meals WHERE is_sandbox=true のレコード数を 1 hour 毎に集計
- 閾値: 通常の 100 倍超で alert

#### 対応
- 該当 user_id の sandbox 行を一括削除 (operator/09-runbook 経由):
  ```sql
  DELETE FROM meals WHERE user_id = $1 AND is_sandbox = true;
  ```
...
- [ ] `meals.is_sandbox` の partial unique index で v1 の二重 INSERT 防止が十分か
- [ ] `user_daily_meals.is_sandbox` も同様 unique 制約か (= Step 2 の sandbox 二重 INSERT 防止)
