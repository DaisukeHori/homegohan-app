...
`should_show` 判定の高速化。pending な user だけ index に乗る。

### 1.2 `meals` に `is_sandbox` 追加
...
```sql
CREATE INDEX idx_meals_user_non_sandbox
...
### 1.3 `user_daily_meals` に `is_sandbox` 追加
...
stateDiagram-v2
  ...
  state "判定" as Eligibility {
    ...
    CheckActivity --> ExistingUserAutoSkip: meals OR user_daily_meals 既存 (non-sandbox)
...
| 遷移 | DB 副作用 |
|---|---|
| Step 1 完了 (Step1Loading → Step2Menu) | `meals` INSERT (`is_sandbox=true`) + `user_badges` INSERT (`first_bite`、未保有時のみ) |
| Step 2 完了 | `user_daily_meals` INSERT + `user_badges` INSERT (`planner`) |
...
-- 概念のみ:
SELECT handson_tour_completed_at INTO v_existing FROM user_profiles WHERE id = $1;
...
- サーバー側: `meals` の重複は厳密には防げない (タイムスタンプで区別)
...
問題点:
- `meals` に `is_sandbox=true` で 1 行追加済み
...
`user_profiles` (PK は `id`、`auth.users(id)` への FK):
```sql
...
- [ ] `meals` テーブルの正確な列名 (operator/01-data-model §3.26.2 確認)
- [ ] `user_daily_meals` テーブルの正確な列名 (同 §3.26.3)
