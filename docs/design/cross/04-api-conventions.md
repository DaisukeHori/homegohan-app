...
**v1 プレフィックス**: 全エンドポイントに付与 (後方互換 6 ヶ月保証)

| ドメイン | パスプレフィックス | 対象ロール |
|---------|-----------------|----------|
| 家族管理 | `/api/v1/family/` | 認証済みユーザー全般 |
| 組織管理 | `/api/v1/org/` | org_member 以上 |
| 運営管理 | `/api/v1/admin/` | support / admin 以上 |
| super_admin 専用 | `/api/v1/super-admin/` | super_admin のみ |
| Webhook 受信 | `/api/v1/webhooks/` | 署名検証付き |
| 公開 API | `/api/v1/public/` | 認証不要 (rate limit あり) |

### 3.2 パス例

```
GET    /api/v1/family/groups                    # 家族グループ一覧
POST   /api/v1/family/groups                    # 家族グループ作成
GET    /api/v1/family/groups/{groupId}           # 詳細
PATCH  /api/v1/family/groups/{groupId}           # 部分更新
DELETE /api/v1/family/groups/{groupId}           # 削除
POST   /api/v1/family/groups/{groupId}/freeze    # 状態変更アクション
GET    /api/v1/org/members                       # 組織メンバー一覧
POST   /api/v1/org/licenses/{poolId}/assignments/bulk  # バルク操作
POST   /api/v1/admin/users/{userId}/ban          # 管理アクション
POST   /api/v1/super-admin/impersonate/{userId}  # super_admin 専用
...
| `FAM_GROUP_DISSOLVED` | 家族グループが解散済み | 404 |
...
- URL パスに `/v1/` を含める (`/api/v1/family/groups`)
- **後方互換の保証期間**: v1 → v2 移行時は v1 を **最低 6 ヶ月** 並行稼働
- **互換を壊す変更**: メジャーバージョンアップ必須
  - フィールドの削除、型変更、エラーコード変更
- **追加は後方互換**: 新フィールド追加、新エンドポイント追加は v1 のまま可
...
| 資産 | 状態 | 対応 |
|------|------|------|
| 旧パス API (`/api/org/users`, `/api/org/settings`) | 削除 | `/api/v1/org/members` 等に置換 |
| camelCase 変換ミドルウェア (既存) | 削除 | snake_case 統一 |
| 既存 `/api/family/groups` | 削除→再作成 | `/api/v1/family/groups` で本仕様準拠 |
