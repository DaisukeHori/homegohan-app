/**
 * generate-single-meal-v2 を叩くための簡易スクリプト（デプロイ後の動作確認用）
 *
 * 例:
 *   node scripts/call-generate-single-meal-v2.mjs --user <uuid> --date 2026-01-05 --meal breakfast --note "カレー以外で"
 *
 * 必須環境変数:
...
function printHelp() {
  console.log(`
使い方:
  node scripts/call-generate-single-meal-v2.mjs --user <uuid> --date YYYY-MM-DD --meal <breakfast|lunch|dinner|snack|midnight_snack> [--note "..."] [--requestId <uuid>]
...
async function main() {
  ...
  const res = await fetch(`${url}/functions/v1/generate-single-meal-v2`, {
    method: "POST",
    headers: {
