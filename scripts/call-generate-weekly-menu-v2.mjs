/**
 * generate-weekly-menu-v2 を叩くための簡易スクリプト（デプロイ後の動作確認用）
 *
 * 例:
 *   node scripts/call-generate-weekly-menu-v2.mjs --user <uuid> --start 2025-12-30 --note "カレー多め"
 *
 * 必須環境変数:
...
function printHelp() {
  console.log(`
使い方:
  node scripts/call-generate-weekly-menu-v2.mjs --user <uuid> --start YYYY-MM-DD [--note "..."] [--requestId <uuid>]
...
async function main() {
  ...
  const res = await fetch(`${url}/functions/v1/generate-weekly-menu-v2`, {
    method: "POST",
    headers: {
