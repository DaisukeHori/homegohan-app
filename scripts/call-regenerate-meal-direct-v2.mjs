/**
 * regenerate-meal-direct-v2 を叩くための簡易スクリプト（デプロイ後の動作確認用）
 *
 * 例:
 *   node scripts/call-regenerate-meal-direct-v2.mjs --user <uuid> --mealId <uuid> --note "揚げ物は避けて"
 *
 * 必須環境変数:
...
function parseArgs(argv) {
  const args = { userId: null, mealId: null, note: null, requestId: null };
...
function printHelp() {
  console.log(`
使い方:
  node scripts/call-regenerate-meal-direct-v2.mjs --user <uuid> --mealId <uuid> [--note "..."] [--requestId <uuid>]
...
async function main() {
  ...
  const res = await fetch(`${url}/functions/v1/regenerate-meal-direct-v2`, {
    method: "POST",
    headers: {
