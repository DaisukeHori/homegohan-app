...
describe('page.tsx: UX2-01 エラー通知の type 分岐', () => {
  it('完了モーダルの描画で successMessage.type に応じてアイコン/色を切り替えていること', () => {
    const idx = pageSource.indexOf('data-testid="success-message-icon"');
    expect(idx, '完了モーダルのアイコン領域が見つかりません').toBeGreaterThan(-1);
    const block = pageSource.slice(idx - 200, idx + 600);
    expect(block).toMatch(/successMessage\.type === 'error'/);
