...
1. **quantityVariantsは必ず2つ以上**生成すること（数量不明の場合を除く）
   - 重量(g)と個数(個/本/枚/切れ/株/束等)の両方を含める
   - 可能なら「パック」「袋」などの購入単位も追加
2. **quantityVariantsは同じ量の異なる表現！** 内訳ではない！
   - ❌ 悪い例: ["約200g（合計）", "120g", "80g"] ← これは内訳なのでNG
   - ✅ 良い例: ["200g", "約1/4個"] ← これは同じ量の異なる表現でOK
3. **kgへの変換は禁止！** 買い物に役立たない
   - ❌ 悪い例: ["320g", "約0.32kg"] ← kg変換は無意味
   - ✅ 良い例: ["320g", "約1/4株"] ← 購入単位への変換が正解
4. 数値がない場合（「適量」「少々」）のみ、variants を1つにしてよい
5. 入力にない材料を追加しない（ハルシネーション禁止）
6. JSONのみ出力（説明文不要）`;
}
...
async function callOpenAI(prompt: string): Promise<NormalizedItem[]> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Fast LLM API error: ${response.status} - ${errorText}`);
  }
