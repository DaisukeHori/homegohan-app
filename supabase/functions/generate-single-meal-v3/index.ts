...
console.log("Generate Single Meal v3 Function loaded (LLM Creative Mode)");
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const logger = createLogger(supabase, "generate-single-meal-v2", userId, logRequestId);
    await logger.info("request_start", { requestId, targetDate, mealType });
    ...
    const aggregatedIngredients = generatedMeal.dishes.flatMap(d => d.ingredients.map(i => `${i.name} ${i.amount_g}g`));
    const dishName = generatedMeal.dishes.map(d => d.name).join("、");
    ...
    await logger.info("request_complete", { savedMealId, dishName });
