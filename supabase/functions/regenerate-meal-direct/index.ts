...
async function regenerateMealBackgroundTask({ 
  ...
  try {
    ...
    // 所有権チェック（service roleでRLSをバイパスするため必須）
    const ownerId =
      (existingMeal as any)?.meal_plan_days?.meal_plans?.user_id ??
      (Array.isArray((existingMeal as any)?.meal_plan_days)
        ? (existingMeal as any).meal_plan_days?.[0]?.meal_plans?.user_id
        : null)

    if (!ownerId) {
      throw new Error('Meal owner not found')
    }
    if (ownerId !== userId) {
      ...
      body: JSON.stringify({
        messages: [
          { role: 'system', content:
            ...
            '目的: 既存の献立を「最小差分で自然に」差し替え、健康上の課題（例: 減塩/栄養不足/偏り）を改善します。\\n' +
            '最優先: アレルギー（絶対除外）/禁忌/健康上の制約を厳守します。\\n' +
            'ナレッジベース（献立サンプル/レシピ）を参照して、食文化として自然な組み合わせにしてください。\\n' +
            '出力は指定スキーマのJSONのみ（Markdown/説明文/コードブロック禁止）。\\n' +
            '数値は概算でよいが、料理合計=totalNutrition など整合性は必ず取ってください。'
          },
